...
// src/plugins/rehype-remove-lonely-p.ts
...
// src/plugins/rehype-unwrap.ts
function rehypeRemoveWrappingP() {
  return (tree) => {
    visit(tree, "element", (node, index, parent) => {
      ...
      if (nonTextChildren.length === 2 && ["div", "img", "video"].includes(nonTextChildren[0].tagName)) {
      ...
      if (node.tagName === "p" && node.children.length === 1 && ["div", "img", "video"].includes(node.children[0]?.tagName)) {
...
// src/plugins/remark-img-auto-import.ts
import { parse } from "acorn";
function remarkImgAutoImport() {
  return (tree) => {
    let i = 0;
    const importNodes = [];
    visit(tree, ["mdxJsxFlowElement", "mdxJsxTextElement"], (node) => {
      ...
      if (typeof srcAttr.value !== "string") return;
      const id = `__img${i++}`;
      const rel = srcAttr.value;
      ...
      const estreeExpr = parse(id, { ecmaVersion: 2020 });
      srcAttr.value = {
  ...
  return (tree) => {
    ...
    for (let index = 0; index < tree.children.length; index++) {
      ...
      if (node.type === "code" && node.lang === "slide") {
        ...
        const frontmatter = js_yaml_default.load(frontmatterContent) || {};
        if (frontmatter.reveal && typeof frontmatter.reveal === "object") {
          frontmatter.reveal = Object.fromEntries(
            Object.entries(frontmatter.reveal).map(([key, value]) => [
...
var remark_slides_default = remarkSlides;
export {
  rehypeRemoveLonelyParagraph,
  rehypeRemoveWrappingP,
  remarkImgAutoImport,
  remark_slides_default as remarkSlides
};
