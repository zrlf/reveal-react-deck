...
import { useDeckStore } from "./useDeck.js";
/**
 ...
 */
function useAnimation(scope, animateFunction, onAction) {
    ...
    if (onAction === "slide") {
        ...
        useEffect(() => {
            if (!isPresent || !useDeckStore.getState().motion)
                return;
            animateFunction();
        }, [isPresent]);
    }
        ...
        useEffect(() => {
            ...
            if (onAction === "fragment") {
                if (scope.current) {
                    const fragmentNumber = Number(scope.current.getAttribute("data-fragment-index"));
            ...
            else if (typeof onAction === "number" &&
                fragment === onAction &&
